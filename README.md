...
3. **miRNA-mRNA matching file** generally contains two columns, which shows the corresponding relationship of the sample identifiers in miRNA expression file and mRNA expression file. It also serves as a index to denote which samples we choose to analyze. It is organized as follows:<br>
...
The first part contains the sequence matching scores for putative miRNA-mRNA pairs, they are obtained from TargetScan and provided as two [data files](https://github.com/PANWANG2014/miRTIGO/blob/master/miRTIGO_algorithm/Sequence%20matching%20scores.7z):CWCS and conserved\_CWCS. The CWCS includes all putative miRNA-mRNA combinations predicted by TargetScan, while the conserved\_CWCS set only includes the conserved predictions.
	
> CWCS_matrix = as.matrix(read.table("`CWCS.txt`", head = TRUE, sep = "\t"))<br>


The other part contains paired miRNA-mRNA expression profiles and should be provided by the users. 

>name\_cancer = as.matrix(read.table("`Sample_miRNA_mRNA_paired.txt`", head = TRUE, sep = "\t"))<br>
>mirna\_cancer = as.matrix(read.table("`Sample_miRNA_expression.txt`", head = FALSE, sep = "\t"))<br>
>mrna\_cancer = as.matrix(read.table("`Sample_mRNA_expression.txt`", head = FALSE, sep = "\t"))<br>	
...
* Input sequence data files<br>

	| Data file | Description | 
	|:-------------:|:-------------| 
	| CWCS\_matrix.txt | _cumulative weighted context++ scores_ (weighted miRNA response elements, wMREs) between an miRNA and mRNA |
	| qMRE\_matrix.txt | number of target sites (quantitative miRNA response elements, qMREs) on one mRNA for one miRNA |
	| qMRE\_con\_matrix.txt | number of conserved target sites (qMREs) on one mRNA for one miRNA|
...
* Files needed to perform evaluation analyses (reference validation data)<br>
	...
	* Known cancer-related miRNAs and genes<br>
    ...
    These reference data files are stored along with relevant source codes.

    Detailed descriptions of all the above datasets are provided in the **Supplementary Note** of the paper.<br>

---
### <a name="6">4. Examples of predicted results</a><br>
miRACLe provides both population-level result and individual-level result, and it outputs the top 1% of MMIs defined by miRACLe score in default. To show as an example, [Here](https://github.com/PANWANG2014/miRTIGO/blob/master/Predicted_results.7z) we provide the detailed population-level results for each of the 32 TCGA cancer types and the individual-level results for each of the 59 NCI-60 cell lines.
